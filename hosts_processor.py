...
class HostsProcessor:
    ...
    def process(self):
        ...
        # Features
        # first seen (ms)
        first_seen_ms = self._host(self.df, 'bidirectional_first_seen_ms', 'bidirectional_first_seen_ms').min().squeeze()
        out_df['first_seen_ms'] = first_seen_ms
        # last seen (ms)
        last_seen_ms = self._host(self.df, 'bidirectional_last_seen_ms', 'bidirectional_last_seen_ms').min().squeeze()
        ...
        sent_packets = self._host(self.df, 'src2dst_packets', 'dst2src_packets').sum().squeeze()
        ...
        tcp_sessions = self._host(self.df[self.df['protocol']==Protocol.TCP.value], 'protocol', 'protocol').count().squeeze()
        out_df['tcp_sessions'] = tcp_sessions
        # number of UDP packets sent
        sent_udp_packets = self._host(self.df[self.df['protocol']==Protocol.UDP.value], 'src2dst_packets', 'dst2src_packets').sum().squeeze()
        out_df['sent_udp_packets'] = sent_udp_packets
        # number of TCP packets sent
        sent_tcp_packets = self._host(self.df[self.df['protocol']==Protocol.TCP.value], 'src2dst_packets', 'dst2src_packets').sum().squeeze()
        out_df['sent_tcp_packets'] = sent_tcp_packets
        # number of DNS packets sent
        sent_dns_packets = self._host(self.df[self.df['protocol']==Protocol.UDP.value], 
